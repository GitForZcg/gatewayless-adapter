...
@Service
@Slf4j
public class AbstractFinanceMD5ServiceImpl implements AbstractFinanceMD5Service {
    ...
    public AbstractFinanceMD5ServiceImpl(AppSecretService appSecretService, BaseUrlConfig config) {
        this.appSecretService = appSecretService;
        this.config = config;
    }

    @Override
    public <T extends DemoFinanceMd5Param> Map<String, Object> execute(T reqDto, BaseNode node) {
        ...
        if (accessKeyConfig == null) {
            throw new BizException("456", "财务MD5加签:获取密钥配置失败");
        }
    ...
    @Override
    public <T extends DemoFinanceMd5Param> Map<String, Object> execute(List<T> reqDto, BaseNode node) {
        String serviceName = node.name().split(Separator.UNDERSCORE)[0].concat(DB_SUFFIX).toLowerCase();
        AccessKeyConfig accessKeyConfig = appSecretService.getSecretByServiceName(serviceName);
        if (accessKeyConfig == null) {
            throw new BizException("456", "MD5加签:获取密钥配置失败");
        }
        long timeStamp = ZonedDateTime.now(ZoneId.of(TIMEZONE)).toEpochSecond();
        Map<String, Object> apiHeader = buildApiHeader(timeStamp, accessKeyConfig.getSecret());
        String requestBody = gson.toJson(reqDto);
        log.info("finance execute request : {}", requestBody);
        return HttpUtil.postEntity(config.getStoreBaseUrl() + node.url(), requestBody, apiHeader);
    }

    @Override
    public <T extends DemoFinanceMd5Param> T executeResult(Map<String, Object> map, Class<T> clazz) {
    ...
    @Override
    public <T extends DemoFinanceMd5Param> List<T> checkImportVoucherExecuteResult(Map<String, Object> map, Class<T> clazz) {
        Object status = map.get(STATUS);
        if (!SUCCESS.equals(status.toString())) {
            log.error("import voucher error : {}, data : {}", map.get(MESSAGE).toString(), map.get(DATA).toString());
            throw new BizException("501", map.get(MESSAGE).toString());
        }
        Object data = map.get(DATA);
        return gson.fromJson(gson.toJson(data), TypeHelperUtil.getType(List.class, clazz));
    }
